...
WEBHOOK = 'https://hooks.slack.com/services/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'
...
def get_stack_update_attachment(cf_message):
    title = 'Stack {stack} is now status {status} reason {statusReason}'.format(
        stack=cf_message['StackName'],
        status=cf_message['ResourceStatus'],
        statusReason=cf_message['ResourceStatusReason'])
